...
const doc = new Document({
  ...
  sections: [{
    ...
    children: [
      ...
      tbl1,
      p([t('1페이지는 캠페인 전체를 한 줄로 연결.', { bold: true }),
         t(' 노출 → 도달 → 링크 클릭 → 페이지 방문 → 예약 유도까지 단계별 잔존을 하나의 흐름으로 배치하고, 예산·광고 성과·웹 행동·예약 유도·효율을 다섯 칸으로 요약. 마지막 칸의 유도당 비용이 캠페인 지속 여부 판단의 기준값.', { color: GY })],
        { before: 100 }),
      ...
      p([t('3페이지는 광고 이후의 행동만 취급.', { bold: true }),
         t(' 방문자 → 의미 있는 행동 → CTA 클릭 → 예약 유도로 이어지는 "방문 후 행동 흐름"이 축. 예약 유도 90명, 방문자 대비 1.11%라는 숫자만 보면 낮아 보이지만 흐름과 함께 보면 사람들이 어느 단계에서 멈추는지가 드러남. 이번 기간에는 방문 직후 단계에서 가장 많이 멈췄고, CTA를 누른 사람의 81.8%는 예약 의사를 표현하는 행동(예약 메시지 보내기·전화 걸기)까지 이어짐.', { color: GY })]),
      ...
      li('세는 단위 ', '— 광고 지표(노출·클릭·비용)는 Meta 집계 기준, 웹 지표(방문자·행동·CTA·예약 유도)는 사람 수(고유 기기) 기준. 한 사람이 여러 번 눌러도 1명.'),
      p(t('화면은 세 페이지 모두 기본적으로 한 장 유지. 3페이지는 ①~④가 스크롤 없이 한 화면에 들어가고, ⑤ 소재별 성과표는 접어 두고 버튼으로 펼침 — 라벨 줄이 이미 1위 소재와 유도율을 들고 있어 첫 화면에는 그 한 줄이면 충분. 상세는 눌렀을 때만 펼침. 광고세트 막대의 방문·CTA·유도율, 퍼널 단계의 전환율 계산식, 소재별 행동률·예약 유도율이 모두 클릭으로 열림. 2페이지의 소재 드릴다운과 3페이지의 전체 ACTION 요약도 같은 방식. 매일 확인하는 화면에 글이 많으면 정작 봐야 할 숫자가 묻히기 때문.', { color: GY }), { before: 70 }),

      h2('6', '운영 활용'),
      tbl2,
      p(t('보는 단위는 페이지마다 다름. 1페이지는 기간 전체 고정 요약으로 보고용, 2페이지는 소재·광고세트 단위로 좁혀 보는 비교용, 3페이지는 기간(전체·1차·2차)뿐 아니라 특정 하루만 골라 보는 일상 점검용. 3페이지에서 하루를 고르면 전일 대비 변화가 함께 표시되며, 방문자와 세션은 변화율(%)로, 행동과 예약 유도는 사람 수 차이로 표시. 하루 예약 의사 표현이 1~12명 수준이라 비율 변화는 표본에 따라 크게 흔들리기 때문.', { color: GY }), { before: 100 }),
      p(t('데이터만으로 원인을 확정할 수 없는 경우 세 페이지 모두 "문제 원인"이 아니라 "우선 점검 영역" 또는 "개선 가설"로 표현.', { color: GY })),

      h2('7', '측정 한계'),
      p(t('한계를 감추지 않고 해석 범위를 명시. 아래 내용은 화면 복잡도를 높이지 않기 위해 대시보드에는 넣지 않고 이 문서에만 기재.', { color: GY })),
      p(t('광고 데이터 (1·2페이지)', { bold: true, size: 17 }), { before: 60, after: 40 }),
      li('도달은 기간 합산 불가. ', 'Meta가 조회 기간마다 중복을 제거해 다시 계산하므로 일자별 도달을 더할 수 없음. 1페이지의 도달 213,946명은 13일 기준값이며 각주로 명시.'),
      li('광고 클릭과 페이지 방문은 서로 다른 시스템의 집계. ', 'Meta의 클릭 수와 웹 계측의 방문자 수는 1:1로 대응하지 않으므로 "클릭→방문 55.5%"는 두 시스템 대조값이며 오차 포함.'),
      p(t('웹 데이터 (3페이지)', { bold: true, size: 17 }), { before: 90, after: 40 }),
      ...
      li('클릭 이후는 확인 불가. ', '메신저 실제 발송 여부, 페이지 내 이탈 지점(스크롤 깊이 미계측) 모두 측정 범위 밖. 이에 따라 퍼널을 "이탈 위치 분석"이 아니라 "방문 후 행동 흐름"으로 정의하고, 내비게이션 클릭도 이탈 위치가 아닌 관심 표시로만 사용.'),
